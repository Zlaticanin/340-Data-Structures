...
template<typename T> print_list<T>::print_list(const unsigned &s, 
					       ...
					       const unsigned &c) {
  sz = s; // heap size
  wid = w; // minimum number of chars written in printout
  len = l; // maximum number of items printed in a single line
  cnt = c; // counter with the default value 0
}
...
template<typename T> void print_list<T>::operator()(const T &x) {
  ...
  if(cnt % len == 0 && cnt != sz) // to print a new line and to check if cnt is at the end of the heap (if so, don't print a new line)
    cout << "\n";

  if(cnt % sz == 0) // printing new line after the output is printed
    cout << "\n\n";
} 
#endif  // HEAP_H_
